...
export default {
  async fetch(request, env) {
    ...
    if (payload.website) {
      return json({ message: "Accepted" }, 200, origin);
    }
    ...
    if (!env.RESEND_API_KEY) {
      return json({ error: "RESEND_API_KEY is not configured" }, 500, origin);
    }

    try {
      await sendViaResend(payload, env);
      return json({ message: "Thanks. Your request has been sent." }, 200, origin);
    } catch (error) {
      return json({ error: error.message || "Unable to send email" }, 502, origin);
    }
  },
};
